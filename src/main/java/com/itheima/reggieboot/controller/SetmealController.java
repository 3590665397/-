...
import com.itheima.reggieboot.dto.SetmealDto;
...
public class SetmealController {
    ...
    @GetMapping("/dish/{id}")
    public R<List<SetmealDishDto>> dishs(@PathVariable Long id){
        List<SetmealDishDto> dishswithimg = setmealDishService.dishswithimg(id);
        return R.success(dishswithimg);
    }

    @PostMapping("status/{status}")
    public R<String> updateDishStatus(@PathVariable int status, @RequestParam List<Long> ids) {
        boolean success = setmealService.updateDishStatusByStatus(status, ids);
